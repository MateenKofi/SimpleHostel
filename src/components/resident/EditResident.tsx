...
import type { ApiError, ResidentDto } from "@/types/dtos";
import { useEffect } from "react";

type EditResidentInputs = z.infer<typeof AdminResidentFormSchema>;

const EditResident = () => {
  const residentStore = useAddedResidentStore((state) => state.resident);
  const navigate = useNavigate();

  // Cast to ResidentDto since we know EditResident is only called with ResidentDto
  const resident = residentStore && 'userId' in residentStore ? residentStore as ResidentDto : null;

  // If no resident is selected, navigate back
  useEffect(() => {
    if (!resident) {
  ...
  const {
    ...
    defaultValues: resident ? {
      ...
      relationship: resident.emergencyContactRelationship || resident.relationship || "",
      gender: resident.user?.gender || resident.gender || "MALE",
    } : undefined,
  });

  const updateResidentMutation = useMutation({
    mutationFn: async (resident_data: EditResidentInputs) => {
      if (!resident) throw new Error("No resident selected");

      const responseData = await updateResident(resident.id, {
        name: resident_data.name,
        email: resident_data.email,
        phone: resident_data.phone,
        gender: resident_data.gender,
        studentId: resident_data.studentId,
        course: resident_data.course,
        emergencyContactName: resident_data.emergencyContactName,
        emergencyContactPhone: resident_data.emergencyContactPhone,
        emergencyContactRelationship: resident_data.relationship,
      });
      return responseData;
    },
    onSuccess: () => {
  ...
  const onSubmit: SubmitHandler<EditResidentInputs> = (values) => {
    updateResidentMutation.mutate(values);
  };
