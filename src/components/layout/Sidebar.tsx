...
const Sidebar = () => {
  ...
  return (
    <div
      className={`min-h-screen bg-background text-foreground transition-all duration-300 ease-in-out relative px-2 mr-16 ${
    ...
    >
      ...
      <button
        ...
        className="absolute right-0 top-3 transform translate-x-1/2 bg-primary p-2 rounded-full border-l-2 "
      >
        {isExpanded ? (
      ...
      <nav className="mt-4">
        {menuItems.map((item, index) => (
          <Link
            key={index}
            to={item.path}
            className="flex text-nowrap text-base font-body items-center px-4 py-2 my-2 text-foreground hover:bg-primary rounded-lg hover:rounded-lg hover:text-primary-foreground transition-colors duration-200"
          >
            <item.icon size={25} />
      ...
      <div className="absolute bottom-4 w-full pr-4">
        <button
          onClick={() => {}}
          className="flex items-center px-4 py-2 w-full text-foreground hover:bg-primary hover:text-primary-foreground hover:rounded-lg transition-colors duration-200 font-body"
        >
          <LogOut size={20} />
